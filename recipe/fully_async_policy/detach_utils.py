...
def prepare_single_generation_data(batch_dict, config) -> DataProto:
    ...
    # These keys may not exist when using return_raw_chat=True (tokenization happens in AgentLoop)
    batch_keys_to_pop = ["input_ids", "attention_mask", "position_ids"]
    non_tensor_batch_keys_to_pop = ["raw_prompt_ids"]

    # Only pop keys that actually exist
    existing_batch_keys = [k for k in batch_keys_to_pop if full_batch.batch is not None and k in full_batch.batch.keys()]
