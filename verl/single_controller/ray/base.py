...
class RayWorkerMixin:
    ...
    def _setup_env_cuda_visible_devices(self):
        # Call the next class in MRO (backend-agnostic HIP/ROCR normalization)
        super()._setup_env_cuda_visible_devices()

        # Ray-specific NOSET logic
...
class RayClassWithInitArgs(ClassWithInitArgs):
    ...
    def __init__(self, cls, *args, **kwargs) -> None:
        ...
        if isinstance(raw_cls, type) and issubclass(raw_cls, Worker) and not issubclass(raw_cls, RayWorkerMixin):
            raw_cls = type(raw_cls.__name__, (RayWorkerMixin, raw_cls), {})
...
class RayWorkerGroup(WorkerGroup):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        # Support both new cls_with_init and deprecated ray_cls_with_init
        if cls_with_init is not None and ray_cls_with_init is None:
...
def create_colocated_worker_cls(class_dict: dict[str, RayClassWithInitArgs]):
    ...
    worker_cls = _determine_fsdp_megatron_base_class(
        [_unwrap_ray_remote(cls.cls).__mro__ for cls in class_dict.values()]
    )
